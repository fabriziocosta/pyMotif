...
class MuscleAlign_wrapper(object):
    ...
    def transform(self, seqs=[]):
        ...
        params = {'maxiters': 7}
        if self.diags is True: params['diags'] = True
        if self.maxhours is not None: params['maxhours'] = self.maxhours
...
class Weblogo(object):
    ...
    def __init__(self,
        ...
        if x_label: options.xaxis_label = x_label
        options.show_yaxis = show_y_axis
        if y_label: options.yaxis_label = y_label
        ...
        if fineprint: options.fineprint = fineprint
